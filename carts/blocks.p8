...
function u_menu()
  ...
  if sel==1 then
    if(btnp(0) and set.startlevel>1)set.startlevel-=1
    if(btnp(1) and set.startlevel<20)set.startlevel+=1
  elseif sel==2 then
  ...
  elseif sel==8 then
    if(btnp(0) and set.garbagelines>0)set.garbagelines-=1
    if(btnp(1) and set.garbagelines<15)set.garbagelines+=1
...
function drawsettings(sel)
  print("options",48,9,7)
  local nowopt={set.startlevel,set.lockspeed,set.spin,set.viewnext,set.hold,set.harddrop,set.blkcnt,set.garbagelines}
  for i=1,#setdisp do
    local nowcolor=7
    if i==sel then
...
function initgame()
  ...
  hold=0
  dummytile()
  if(set.garbagelines>0)garbagetile(set.garbagelines)
  gh=getghost(np)
  --presettile(ptiles[1])
  --frames to next auto drop
  t_drop=spd_drop[set.startlevel]
...
function updatetimer()
  if np.x==gh.x and np.y==gh.y then
    t_lock-=1
    t_drop=spd_drop[sc.level]
  else
    t_lock=spd_lock[1]
    t_drop-=1
    if spd_drop[sc.level]==0 then
      moveblk(4)
...
--for garbage tiles
function garbagetile(lines)
  for r=pa.h,pa.h-lines+1,-1 do
    local l=0
    for c=1,pa.w do
      if rnd(1)>0.35 and l<10 then
        b[r][c]=flr(rnd(8))
        l+=1
      end
    end
...

__music__
01 04514311
00 05464311
00 06424311
